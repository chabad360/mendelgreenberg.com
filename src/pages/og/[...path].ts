...
export const GET: APIRoute = async ({ props: { item }, url }) => {
	...
	return await satoriAstroOG({
		template: html` <div
      style="display: flex; flex-direction: column; background-color: ${background}; color: ${color}; height: 630px; width: 1200px; justify-content: center; text-align: left; padding: 0 100px;"
    >
      <div
        style="display: flex; flex-direction: column; justify-content: center; align-items: stretch; border-left: 2px solid ${color}; padding: 1rem 2rem;"
      >
        <small
          style="font-family: ${serifFamily}; margin: 0; font-size: 1rem; color: ${color};"
        >
          ${item.data.date.toLocaleDateString()} ⋅
          ${getReadingTime(item.body).text}
        </small>
        <h1
          style="font-family: ${sansFamily}; margin-bottom: 0; font-size: 4rem;"
        >
          ${item.data.title}
        </h1>
        <h2
          style="font-family: ${serifFamily}; margin-top: 0.25rem; font-size: 2.5rem;"
        >
          ${item.data.description}
        </h2>
        <p
          style="font-family: ${serifFamily}; margin: 0; margin-top: 2rem; font-size: 1.5rem;"
        >
          ${config.title}
        </p>
      </div>
